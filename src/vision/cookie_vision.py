...
class CookieVision:
    ...
    def get_upgrade(self):
        ...
        v_channel = hsv[:, :, 2] # Brilho
        
        v_channel = cv2.subtract(v_channel, (s_channel * 0.1).astype(np.uint8))
        v_channel = cv2.convertScaleAbs(v_channel, alpha=0.3, beta=60)

        rectangle_y, rectangle_h, rectangle_x, rectangle_w = 17, 35, 17, 35
        roi_upgrade = v_channel[rectangle_y:rectangle_h, rectangle_x:rectangle_w]
        brilho_upgrade = np.mean(roi_upgrade)

        pode_comprar = brilho_upgrade >= 105
    ...
    def get_structure(self):
        ...
        for name, template in self.templates_structures.items():
            ...
            if max_val >= 0.8:
                ...
                can_buy = brightness_mean > 194
                
                if self.debug:
                    print(f"brilho da estrutura {name}: {brightness_mean}")
